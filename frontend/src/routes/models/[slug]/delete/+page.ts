...
export const load: PageLoad = async ({ fetch, params }) => {
	// Auth is server-authoritative; bounce anonymous users to login before
	// they see a confirmation screen they can't submit.
	...
	const res = await fetch(`/api/models/${params.slug}/delete-preview/`);
	if (res.status === 404) {
