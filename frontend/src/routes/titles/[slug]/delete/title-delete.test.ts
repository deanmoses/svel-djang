...
describe('title submitDelete', () => {
	...
	// the right endpoint.
	it('posts to /api/titles/{slug}/delete/', async () => {
		...
		expect(POST).toHaveBeenCalledWith('/api/titles/{slug}/delete/', expect.anything());
		expect(POST.mock.calls[0][1].params.path.slug).toBe('g');
