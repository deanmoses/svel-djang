<script lang="ts">
	...
	function toFormFields(s: typeof system) {
		return {
			name: s.name,
	...
	let editFields = $state(untrack(() => toFormFields(data.system)));
	let editNote = $state('');
	...
	function getChangedFields(): Record<string, unknown> {
		const original = toFormFields(system);
	...
	async function saveChanges() {
		...
		const { data: updated, error } = await client.PATCH('/api/systems/{slug}/claims/', {
			params: { path: { slug: system.slug } },
