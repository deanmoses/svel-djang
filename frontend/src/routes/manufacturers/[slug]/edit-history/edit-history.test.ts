...
describe('manufacturer edit-history SSR route', () => {
	it('loads edit history from the backend API', async () => {
		...
		const result = await load({
			fetch,
			url: new URL('http://localhost:5173/manufacturers/williams/edit-history'),
		...
		expect(request.url).toBe('http://localhost:5173/api/edit-history/manufacturer/williams/');
	});
