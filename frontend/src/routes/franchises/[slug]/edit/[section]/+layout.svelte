<script lang="ts">
	...
	$effect(() => {
		...
		const segment = currentSection?.segment ?? defaultFranchiseSectionSegment();
		goto(resolve(`/franchises/${slug}?edit=${segment}`), { replaceState: true });
	});
</script>

{#if isMobile === true}
	<EditSectionShell
		detailHref={resolve(`/franchises/${slug}`)}
		{switcherItems}
