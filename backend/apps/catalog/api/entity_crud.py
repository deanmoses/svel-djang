    ...
    * ``parent_field`` — set on subgen/subtype so the preview surfaces the
    ...
    def _delete_preview(
        ...
        if parent_field is not None:
            ...
            parent_ref = EntityRef(name=parent.name, slug=parent.public_id)
...
def register_entity_create[ModelT: CatalogModel, SchemaT: Schema](
    ...
    *,
    ...
    ``POST /{parent_public_id:path}/<route_suffix>/`` on the *parent's*
    ...
    *include_deleted_name_check* forwards to ``assert_name_available``'s
    ...
    if parented:
        ...
        parent_public_id_field = parent_model.public_id_field
