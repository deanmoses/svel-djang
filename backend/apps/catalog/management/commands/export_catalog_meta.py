    ...
    def handle(self, **options):
        ...
        for ct in ContentType.objects.filter(app_label="catalog").order_by("model"):
            ...
            if not model_class or not hasattr(model_class, "link_url_pattern"):
                continue
            type_key = getattr(model_class, "api_type_key", ct.model)
            entity_types.append((type_key, model_class._meta.verbose_name.title()))
            cats = getattr(model_class, "MEDIA_CATEGORIES", [])
            if cats:
                key = getattr(model_class, "api_type_key", ct.model)
