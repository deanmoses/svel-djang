...
### Follow-ups surfaced by the Step 7.1 first-item landing

Not blocking, but worth capturing while the context is fresh. Land next time [entity_crud.py](backend/apps/catalog/api/entity_crud.py) or the systems/people serializers are touched.

- **Make the entity_crud registrars generic over `(ModelT, SchemaT)`.** Today `SerializeFn = Callable[[Any], Schema]` and `register_entity_delete_restore` / `register_entity_create` accept `model_cls: type[CatalogModel]`, `detail_qs: Callable[[], Any]`, `serialize_detail: SerializeFn`, `response_schema: type[Schema]` — four args that are contractually linked but not type-linked. Nothing stops a future edit from pairing `_serialize_theme_detail` with `response_schema=FranchiseDetailSchema`; mypy stays silent and the failure surfaces as a pydantic validation error at request time (or a silently-wrong OpenAPI). Introduce `ModelT = TypeVar("ModelT", bound=CatalogModel)` and `SchemaT = TypeVar("SchemaT", bound=Schema)`, then type the registrars as `(type[ModelT], detail_qs: Callable[[], QuerySet[ModelT]], serialize_detail: Callable[[ModelT], SchemaT], response_schema: type[SchemaT], ...)`. Call sites don't change — mypy infers both from the explicit args. Main wrinkle: [taxonomy.py](backend/apps/catalog/api/taxonomy.py)'s `detail_qs=lambda cls=model_cls: _taxonomy_detail_qs(cls)` default-arg pattern (the pre-existing "Cannot infer type of lambda" baseline entries at lines 205/224) would need a reshape or a typed helper — arguably an improvement since that's _why_ `DetailQsFn` is `Callable[[], Any]` today.
- **Restructure the accumulator-then-mutate pattern in `_serialize_system_detail` and `_serialize_person_detail`.** Both helpers build a `dict[str, SomeSchema]` and then mutate the Schema instances in place (`titles[key].thumbnail_url = ...`, `titles[key].roles.append(...)`) as more data arrives in the loop. Pydantic v2 allows this (no `validate_assignment`), but it's off-idiom — pydantic models are usually treated as immutable after construction. Build mutable state (dataclass or plain dict) during accumulation, then construct the Schema once at the end. The refactor is mechanical but didn't fit in the first-item scope.
- **`list_all_systems` in [systems.py](backend/apps/catalog/api/systems.py) still returns `list[dict[str, Any]]`.** Not touched in the 7.1 first item (only the detail path was in scope), but it's the same "return Schema not dict" debt — the response is `list[SystemListSchema]` and the dicts match that shape. Fold into Step 2.2 residual cleanup next time the file is open.
