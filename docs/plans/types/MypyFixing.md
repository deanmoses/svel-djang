...
- **Clear the incremental cache when the type system changes.** Adding a type alias, changing an override, or renaming a module-level symbol can leave `.mypy_cache` reporting wrong errors. Fix: `rm -rf backend/.mypy_cache` and re-run.
